...
#from torch.utils.tensorboard import SummaryWriter    #aggiunto per tensorboard
...
def evaluate(model, valid_loader, class_weights, device):
    ...
    #inizio
    # Calcolo accuracy classica dalla confusion matrix
    tn, fp, fn, tp = cm.ravel()
    acc = (tp + tn) / cm.sum()
    print(f"AUC={auc}, F1={f1}, Accuracy={acc}")
    #fine
...
def train_validate(args):
    #summarywriter = SummaryWriter(args.out)      #aggiunto per tensorboard
...
def parse_args():
    ...
    parser.add_argument('--epochs', type=int, default=3)     #rimesse bene 6 
